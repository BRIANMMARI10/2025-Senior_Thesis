...
async def main(name: str, address: str, characteristic: str, led_characteristic: str, use_bdaddr: bool, queue: Queue):
    ...
    if not device:
        logger.error("Could not find device with name '%s' or address '%s'", name, address)
        return

    logger.info("Connecting to device...")

    async with BleakClient(device, use_bdaddr=use_bdaddr) as client:
...
if __name__ == "__main__":
    ...
    asyncio.run(
        main(
            ...
            led_characteristic=args.led_characteristic,
